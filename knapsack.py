#!/usr/bin/env python3
def profit(weights, prices, size, n, cache):
    if size == 0 or n == 0:
        return 0
    if cache[size][n] != 0:
        return cache[n][size]
    if size >= weights[n - 1]:
        cache[size][n] = max(profit(weights, prices, size, n - 1, cache),
                             prices[n - 1] + profit(weights, prices, size - weights[n - 1], n - 1, cache))
    else:
